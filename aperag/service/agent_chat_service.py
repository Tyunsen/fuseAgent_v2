...
TRAILING_SOURCE_SECTION_PATTERN = re.compile(
...
class AgentChatService:
    ...
    @staticmethod
    def _get_progress_message(language: Optional[str]) -> str:
        return "Searching knowledge sources and preparing the answer..."
    ...
    def _sanitize_response_content(content: str) -> str:
        ...
        cleaned = THINK_BLOCK_PATTERN.sub("", content)
        cleaned = TRAILING_SOURCE_SECTION_PATTERN.sub("", cleaned)
        marker_indexes = [cleaned.find(marker) for marker in TRAILING_SOURCE_MARKERS if marker in cleaned]
        if marker_indexes:
    ...
    @staticmethod
    def _split_stream_chunks(content: str, chunk_size: int) -> List[str]:
        if not content:
            return []

        sentence_boundaries = "".join(
            [chr(0x3002), chr(0xFF01), chr(0xFF1F), "!", "?", chr(0xFF1B), ";"]
        )
        pattern = f"(\\n{{2,}}|(?<=[{re.escape(sentence_boundaries)}])\\s+)"
    ...
    async def _stream_message_content(
        self, message: Dict[str, Any], websocket: WebSocket, chunk_size: int = 48, delay: float = 0.02
    ) -> None:
        """
        ...
        chunks = self._split_stream_chunks(content, chunk_size)
        if not chunks:
            chunks = [content]
