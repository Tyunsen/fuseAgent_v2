...
from aperag.mirofish_graph.helpers import build_graph_status_message
...
class CollectionService:
    ...
    async def _resolve_default_model(
        ...
        *,
        api_type: str,
        default_tag: str,
    ) -> view_models.ModelSpec:
        from aperag.service.llm_available_model_service import llm_available_model_service

        tag_filters = [
            view_models.TagFilterRequest(
                tag_filters=[view_models.TagFilterCondition(operation="AND", tags=[default_tag])]
            ),
            view_models.TagFilterRequest(
                tag_filters=[view_models.TagFilterCondition(operation="AND", tags=["enable_for_collection"])]
            ),
        ]

        for tag_filter_request in tag_filters:
            models = await llm_available_model_service.get_available_models(user_id, tag_filter_request)
            for provider in models.items or []:
                provider_models = getattr(provider, api_type, []) or []
        ...
        raise ValidationException(f"No suitable default {api_type} model found for collection creation")

    async def _build_mirofish_collection_config(
        self,
        user_id: str,
        incoming_config: view_models.CollectionConfig | None,
    ) -> view_models.CollectionConfig:
        embedding_model = incoming_config.embedding if incoming_config and incoming_config.embedding else None
        completion_model = incoming_config.completion if incoming_config and incoming_config.completion else None

        if embedding_model is None:
            embedding_model = await self._resolve_default_model(
                user_id,
                api_type="embedding",
                default_tag="default_for_embedding",
            )
        if completion_model is None:
            completion_model = await self._resolve_default_model(
                user_id,
                api_type="completion",
        ...
        return view_models.CollectionConfig(
            ...
            knowledge_graph_config=(incoming_config.knowledge_graph_config if incoming_config else None),
            index_prompts=(incoming_config.index_prompts if incoming_config else None),
            creation_mode=MIROFISH_CREATION_MODE,
            graph_engine=MIROFISH_GRAPH_ENGINE,
            graph_status=GRAPH_STATUS_WAITING_FOR_DOCUMENTS,
            graph_status_message=build_graph_status_message(GRAPH_STATUS_WAITING_FOR_DOCUMENTS),
            graph_revision=0,
    ...
    async def create_collection(self, user: str, collection: view_models.CollectionCreate) -> view_models.Collection:
        collection_type = collection.type or db_models.CollectionType.DOCUMENT
        ...
        async def _create_collection_with_quota(session):
            ...
            config_str = dumpCollectionConfig(collection_config)
        ...
        if collection_config.enable_summary:
