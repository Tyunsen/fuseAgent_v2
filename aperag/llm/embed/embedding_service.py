...
_MINIMAX_EMBEDDING_URL_SUFFIX = "/embeddings"
...
class EmbeddingService:
    ...
    def _is_minimax_embedding(self) -> bool:
        return (
            self.model == _MINIMAX_EMBEDDING_MODEL
            and bool(self.api_base)
            and "api.minimaxi.com" in self.api_base
        )
    ...
    def embed_query(self, content: str) -> List[float]:
        ...
        try:
            if self._is_minimax_embedding():
                return self._embed_minimax_batch([content], embedding_type="query")[0]
    ...
    def _embed_batch(self, batch: Sequence[str]) -> List[List[float]]:
        ...
        try:
            if self._is_minimax_embedding():
                return self._embed_minimax_batch(batch, embedding_type="db")
    ...
    def _embed_minimax_batch(self, batch: Sequence[str], embedding_type: str) -> List[List[float]]:
        url = f"{self.api_base.rstrip('/')}{_MINIMAX_EMBEDDING_URL_SUFFIX}"
        payload = {
            "model": self.model,
            "texts": list(batch),
            "type": embedding_type,
        ...
        try:
            ...
            raise EmbeddingError(
                f"MiniMax embedding request failed: {str(e)}",
                {"provider": self.embedding_provider, "model": self.model, "api_base": self.api_base},
            ) from e

        base_resp = data.get("base_resp") or {}
        if base_resp.get("status_code") not in (None, 0):
            raise EmbeddingError(
                f"MiniMax embedding request failed: {base_resp.get('status_msg', 'unknown error')}",
                {"provider": self.embedding_provider, "model": self.model, "response": data},
            )

        vectors = data.get("vectors")
        if not isinstance(vectors, list) or not vectors:
            raise EmbeddingError(
                "MiniMax embedding response did not include vectors",
                {"provider": self.embedding_provider, "model": self.model, "response": data},
            )
