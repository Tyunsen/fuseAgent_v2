...
async def web_search(
    ...
    try:
        ...
        search_llms_txt = (search_llms_txt or "").strip()

        # Small local models occasionally emit empty web_search tool calls for casual chat.
        # Return an empty result instead of bubbling a 400 so the agent can continue and reply.
        if not query and not search_llms_txt:
            return WebSearchResponse(query="", results=[], total_results=0, search_time=0.0).model_dump()
