...
class CollectionConfig(BaseModel):
    ...
    creation_mode: Optional[Literal['aperag_advanced', 'mirofish_simple']] = Field(
        None, description='Collection creation workflow mode'
    )
    graph_engine: Optional[Literal['lightrag', 'mirofish']] = Field(
        None, description='Graph engine used by this collection'
    )
    graph_status: Optional[
        Literal['waiting_for_documents', 'building', 'updating', 'ready', 'failed']
    ] = Field(None, description='Current graph lifecycle status')
    graph_status_message: Optional[str] = Field(
        None, description='Human-readable graph lifecycle summary'
    )
    graph_error: Optional[str] = Field(
        None, description='Last graph build error message'
    )
    graph_revision: Optional[int] = Field(
        0, description='Latest requested graph build revision'
    )
    active_graph_revision: Optional[int] = Field(
        None, description='Latest active graph revision'
    )
    active_graph_id: Optional[str] = Field(
        None, description='Active graph identifier for graph reads'
    )
    graph_last_synced_at: Optional[datetime] = Field(
        None, description='Last successful graph synchronization time'
    )
...
class AnswerGraphReferenceInput(BaseModel):
    source_row_id: str = Field(..., description='Stable source row identifier')
    text: Optional[str] = Field(
        None, description='Displayed passage text for chunk matching fallback'
    )
    document_id: Optional[str] = Field(
        None, description='Optional source document identifier'
    )
    document_name: Optional[str] = Field(
        None, description='Optional source document display name'
    )
...
class AnswerGraphRequest(BaseModel):
    references: list[AnswerGraphReferenceInput] = Field(
        default_factory=list,
        description='Answer evidence rows used to derive the compact answer graph',
    )
    max_nodes: Optional[conint(ge=1, le=100)] = Field(
        24,
        description='Maximum number of nodes to return for answer-scoped rendering',
    )


class AnswerGraphResponse(BaseModel):
    nodes: list[GraphNode] = Field(
        default_factory=list,
        description='Answer-scoped graph nodes',
    )
    edges: list[GraphEdge] = Field(
        default_factory=list,
        description='Answer-scoped graph edges',
    )
    linked_row_ids: list[str] = Field(
        default_factory=list,
        description='Source row identifiers represented in the graph payload',
    )
    is_empty: bool = Field(
        False,
        description='Whether no answer-scoped graph could be resolved',
    )
