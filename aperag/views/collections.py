...
@router.post("/collections/{collection_id}/answer-graph", tags=["graph"])
async def get_answer_graph_view(
    request: Request,
    collection_id: str,
    answer_graph_request: view_models.AnswerGraphRequest,
    user: User = Depends(required_user),
) -> view_models.AnswerGraphResponse:
    """Get an answer-scoped graph derived from cited answer evidence."""
    from aperag.service.answer_graph_service import answer_graph_service

    try:
        return await answer_graph_service.get_answer_graph(
            str(user.id),
            collection_id,
            answer_graph_request,
        )
    except CollectionNotFoundException:
        raise HTTPException(status_code=404, detail="Collection not found")
    except ValueError as e:
