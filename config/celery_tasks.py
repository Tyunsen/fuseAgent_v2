...
@app.task(bind=True, autoretry_for=(Exception,), retry_kwargs={'max_retries': 2, 'countdown': 60})
def mirofish_collection_graph_task(self, collection_id: str, target_revision: int) -> Any:
    """
    Build or update the active MiroFish graph for a collection.

    Args:
        collection_id: Collection ID to build/update
        target_revision: Revision number prepared by the application service
    """
    from aperag.service.mirofish_graph_service import mirofish_graph_service

    try:
        result = mirofish_graph_service.build_graph_for_collection(collection_id, target_revision)
        logger.info(
            "MiroFish graph task completed for %s at revision %s",
            collection_id,
            target_revision,
        )
        return result
        ...
        logger.error(
            "MiroFish graph task failed for %s at revision %s: %s",
            collection_id,
            target_revision,
            e,
            exc_info=True,
        ...
        if self.request.retries >= self.max_retries:
            mirofish_graph_service.handle_build_failure(collection_id, target_revision, str(e))
