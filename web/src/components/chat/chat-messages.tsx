...
export const ChatMessages = ({ chat }: { chat: ChatDetails }) => {
      ...
      onMessage: (message) => {
        ...
        setMessages((msgs) => {
          ...
          if (parts) {
            if (fragment.type === 'start') {
            ...
            } else if (fragment.type === 'references') {
              ...
              const references = Array.isArray(fragment.data)
                ? (fragment.data as Reference[])
                : [];
  ...
  const getMessageKey = (parts: ChatMessage[], index: number) =>
    parts
      .map(
        (part, partIndex) =>
          part.part_id ||
          part.id ||
          `${part.role}:${part.type}:${part.timestamp ?? 'na'}:${partIndex}`,
      )
      .find(Boolean) || `message-group-${index}`;
  ...
  return (
    <div className="flex flex-col gap-6 pb-70">
      {messages.map((parts, index) => {
        ...
        const isAIPending =
          isLoading &&
          parts.filter((p) => p.type === 'message' || p.type === 'error')
            .length === 0 &&
          isAI;

        return (
          <motion.div
            key={getMessageKey(parts, index)}
