...
export const SearchTest = ({
  ...
  const [indexTypes, setIndexTypes] = useState<{
    ...
    graph: {
      available: canUseLegacyGraphSearch(collection.config),
      checked: false,
    },
    summary: {
  ...
  const handleSubmit = useCallback(
    async (values: z.infer<typeof searchParams>) => {
      if (disabled || queryAccess.state === 'blocked' || !collection.id) {
        return;
      }
    ...
    <Dialog open={visible} onOpenChange={setVisible}>
      ...
      <DialogContent>
        <Form {...form}>
          ...
          >
            <DialogHeader>
              <DialogTitle>{getQaEntryLabel(locale)}</DialogTitle>
              <DialogDescription>
                {queryAccess.state === 'warning'
                  ? queryAccessCopy.description
                  : isMirofish
                    ? locale === 'zh-CN'
                      ? '系统默认使用向量与全文结果进行问答。'
                      : 'This flow uses vector and full-text retrieval for Q&A by default.'
                    : ''}
              </DialogDescription>
            </DialogHeader>
